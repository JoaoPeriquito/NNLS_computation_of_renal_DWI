function [locs pks]=peakseekTG(x,minpeakdist,minpeakh)
...
% TG: now includes maxima at the limits of the curves

if size(x,2)==1, x=x'; end

% Find all maxima and ties
locs=find(x([2:end end])<=x(1:end) & x([1 1:end-1])<=x(1:end)); % points before and after are smaller
